...
This guide outlines the process for setting up **CAST Imaging** in a **GCP Kubernetes Cluster environment** using Helm charts.
...
- AKS environment
	...
	- Retrieve cluster credentials: gcloud container clusters get-credentials my-cluster --zone my-cluster-zone --project my-project-id
...
**1. Prepare and Run the installation**
 ...
 - Prepare Ingress Service, Application Gateway or a web server (e.g., NGINX) as a reverse proxy to host the console-gatewayservice (with a DNS i.e castimagingv3.com). The DNS should also have an SSL certificate.
...
## Setup A Google Cloud Filestore for the analysis-node(s) (OPTIONAL)

All pods will use block storage by default.
For the console-analysis-node StatefulSet, it is however possible to configure Google Cloud Filestore (based on file.csi.gcp.com driver) to enable file sharing between analysis nodes, when multiple analysis node are needed.
...
If you do not want use the CAST postgres server preconfigured in this helm chart, you can disable it and configure an Google Cloud SQL for PostgreSQL instead:
- Setup your Google Cloud SQL for PostgreSQL database (postgres 15 - 8GB RAM minimum recommended )
